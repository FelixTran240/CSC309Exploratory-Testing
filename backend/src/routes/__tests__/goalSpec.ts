...
describe("Goal API Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    goal.user_id = returnedUser.id as unknown as number;
  });
  ...
  describe("Test CRUD API Methods", () => {
    it("should create a new goal", async () => {
      const res = await request
        ...
        .send(goal);
      const { user_id, calories, protein, fat, carbs } = res.body.data;
      ...
      goal.id = res.body.data.id;
    });

    it("Should fail to create due to user_id being unique", async () => {
      const res = await request
        .post("/api/goals/")
        .set("content-type", "application/json")
        .set("Authorization", `Bearer ${token}`)
